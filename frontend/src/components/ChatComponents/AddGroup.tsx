...
function AddGroup({
  ...
  const colourOptions = chatEligibleUsers.map((user: any) => ({
    value: user._id,
    label: user.userName,
  }));
  ...
  const handleSubmit=async()=>{
    setIsLoading(true);
    setSelectedUsers([...selectedUsers,{value:userId,label:user.userName}])
        try {
        ...
        <Modal.Body>
          <div className="space-y-6">
              ...
              <div>
                ...
                <div className="mb-4">
                  {croppedImage && (
                    <img src={croppedImage} className="w-full" />
                  )}
                  <Button
