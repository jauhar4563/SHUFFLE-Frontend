...
function Story() {
  ...
  useEffect(() => {
    ...
    getUserStory(userId).then((response: any) => {
  ...
  return (
    <div className="max-w-full mx-auto p-2 bg-white rounded-lg">
      <ul className="flex space-x-6 font-serif">
        <li className="flex flex-col items-center space-y-1 relative">
            ...
            <div className={`${useStoryBg} p-0.5 rounded-full`}>
              <div className="bg-white block rounded-full p-0.5  hover:scale-110 transform transition">
                <img
                  ...
                  src={
                    userStory
                      ? userStory.stories[0].imageUrl
                      : "https://i.pinimg.com/564x/3c/0e/06/3c0e06920dd0d35128763ef0cc3403d7.jpg"
                  }
                  alt="cute kitty"
                />
              </div>
          ...
          <button
            ...
            className="absolute bottom-5 cursor-pointer right-4 bg-gradient-to-b from-purple-600 to-blue-400 rounded-full h-6 w-6  text-white font-semibold border-2 border-white flex justify-center items-center font-mono hover:bg-blue-700"
          >
            <Plus size={15} />
          </button>
        ...
        {stories.length !== 0 &&
          stories.map((story: any) => {
            const isAllRead = story.stories.every((s: any) =>
              s?.views?.includes(userId)
            );
      ...
      {viewStory && (
        <ViewStory story={viewStory} onClose={handleCloseViewStory} />
      )}
      {addStoryModal && (
        <AddStoryModal
