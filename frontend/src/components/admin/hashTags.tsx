...
import { addHashTags, adminHashtagBlock, getHashtags } from "../../services/api/admin/apiMethods";
...
function HashTags() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await addHashTags({ hashtag: tag }).then((response:any)=>{
        const hashtagData = response.data;
        setHashtags(hashtagData.hashtags);
        setTag('');
  ...
  const handleHashtagBlock = (hashtagId: string,status:string) => {
    try {
      const requestData = { hashtagId };
      adminHashtagBlock(requestData)
        .then((response: any) => {
          ...
          if(status=="block"){
            ...
            setHashtags(prevUsers =>
            prevUsers.map(hashtag => {
